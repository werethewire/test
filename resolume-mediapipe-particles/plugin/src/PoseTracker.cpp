...
void PoseTracker::SetMirror( bool m )
{
	mirror = m;
}

void PoseTracker::SetTransform( float s, float ox, float oy )
{
	scale   = s;
	offsetX = ox;
	offsetY = oy;
...
void PoseTracker::Update( const PoseFrame* frame, float dt )
{
	...
	float target   = alive ? 1.0f : 0.0f;
	// Fade in quickly, out slowly: a dropped frame should not blink the figure.
	presence       = Approach( presence, target, alive ? 14.0f : 3.0f, dt );
	if( presence < 0.0005f )
	{
		presence = 0.0f;
		if( !alive )
...
void PoseTracker::RebuildEmissionTable()
{
	...
	boneCdf[ NUM_BONES - 1 ] = 1.0f;// guard against fp drift at the top end
}
