...
static void FillPose( float* lm, float t )
{
    for( int i = 0; i < mpp::NUM_LANDMARKS; ++i ) { lm[i*4+0]=0.5f; lm[i*4+1]=0.5f; lm[i*4+2]=0.f; lm[i*4+3]=1.f; }
    auto set=[&](int i,float x,float y){ lm[i*4+0]=x; lm[i*4+1]=y; };
...
int main( int argc, char** argv )
{
    ...
    {
        mpp::PoseFrame pf; pf.present = true; FillPose(pf.lm, t);
        tracker.Update(&pf, dt);
        t += dt;
    ...
    printf("lit pixels: %d / %d (%.1f%%)\n", nonZeroPixels, W*H, 100.0*nonZeroPixels/(W*H));

    // Write a PPM (flipped, since GL origin is bottom-left).
    FILE* f = fopen( argc > 1 ? "headless_alt.ppm" : "headless_out.ppm", "wb" );
    fprintf(f,"P6\n%d %d\n255\n",W,H);
    for(int y=H-1;y>=0;--y) for(int x=0;x<W;++x){ const unsigned char* p=&pixels[(y*W+x)*4]; fwrite(p,1,3,f); }
    fclose(f);

    ps.DeInit();
    printf("%s\n", nonZeroPixels > 200 ? "PASS: particles rendered" : "FAIL: frame is empty");
    return nonZeroPixels > 200 ? 0 : 1;
}
